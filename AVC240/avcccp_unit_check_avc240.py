...
def run_unit_check():
    ...
    for html_file in assignment_files:
        ...
        if total_images > 0 and height_200_count >= total_images * 0.8:
    ...
    for category, items in checks.items():
        ...
        for check, result in items.items():
            status = "✓" if (result.startswith(str(len(assignment_files))) or result == "Yes") else "⚠"
    ...
    for category, items in checks.items():
        for result in items.values():
            if result.startswith(str(len(assignment_files))) or result == "Yes":
                passed_checks += 1
    ...
    if compliance_percentage >= 95:
        print("🎉 AVC240 is AVCCCP COMPLIANT and ready for Canvas deployment!")
    elif compliance_percentage >= 85:
        print("⚠️  AVC240 is mostly compliant but needs minor fixes")
    else:
        print("❌ AVC240 needs significant work to achieve AVCCCP compliance")
