...
public class avoidanceGameManagerComplete : MonoBehaviour
{
    ...
    void Update()
    {
        if (interactionData.runOver)
        {
            ...
            if(DefeatFX != null){
                Instantiate(DefeatFX, currentPlayer.transform.position, Quaternion.identity);
            }
        ...
        if (interactionData.achievedGoal)
        {
            ...
            if (VictoryFX != null)
            {
                Instantiate(VictoryFX, currentPlayer.transform.position, Quaternion.identity);
            }
            AudioSource musicObject = Music.GetComponent<AudioSource>();
            musicObject.Stop();
            currentPlayer.GetComponent<movementArcade>().enabled = false;
            currentPlayer.GetComponent<Collider2D>().enabled = false;
            interactionData.achievedGoal = false;
        }
    }
